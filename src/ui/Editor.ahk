...
class Editor {
    ...
    static Build() {
        ...
        title := this.orig ? "Modifier le raccourci" : "Nouveau raccourci"
        ...
        ; --- application ---
        UI.Text(g, "Application", M + 2, yApp - 30, 200, 26, T.text, T.bg, 10, 600)
        if hasApp {
            ...
            UI.Text(g, "Aucune application choisie", M + 70, yApp + 14, cw - 86, 24, T.text, T.card, 11, 600)
            UI.Text(g, "Ouvre ton appli, puis choisis-la dans la liste", M + 70, yApp + 38, cw - 86, 20, T.text3, T.card, 9)
        }
        UI.Button(g, "Choisir une appli ouverte", M + 16, yApp + 72, 222, hasApp ? "secondary" : "primary", T.card, (*) => this.PickOpen(), Chr(0xE71D))
        UI.Button(g, "Parcourir…", M + 16 + 222 + 8, yApp + 72, 126, "secondary", T.card, (*) => this.Browse(), Chr(0xE8B7))

        ; --- touche ---
        UI.Text(g, "Touche", M + 2, yLblKey, 200, 26, T.text, T.bg, 10, 600)
        if this.capturing {
            UI.Text(g, "Appuie sur ta combinaison de touches…", M + 26, yKey + 21, cw - 24 - 110 - 40, 22, T.accentText, T.card, 10)
            UI.Button(g, "Annuler", W - M - 16 - 110, yKey + 16, 110, "secondary", T.card, (*) => KeyCapture.Cancel())
            UI.Text(g, "Échap pour annuler · Exemple : Ctrl + Alt + C", M + 16, yKey + 64, cw - 32, 22, T.text3, T.card, 9)
        } else {
            kc := Widgets.Keycaps(Keys.Labels(d.key), T.card)
            if kc.w
                g.Add("Picture", Format("x{} y{} w{} h{}", M + 16, yKey + 19, kc.w, kc.h), "HBITMAP:" kc.hbm)
            else
                UI.Text(g, "Aucune touche choisie", M + 16, yKey + 18, 240, 30, T.text3, T.card, 10)
            UI.Button(g, "Changer", W - M - 16 - 110, yKey + 16, 110, "secondary", T.card, (*) => this.StartCapture(), Chr(0xE765))
            if d.key = Keys.Copilot
                UI.Text(g, "La touche dédiée à côté de la barre d'espace, sur les claviers récents", M + 16, yKey + 64, cw - 32, 22, T.text3, T.card, 9)
            else
                UI.Link(g, "Utiliser la touche Copilot", M + 16, yKey + 64, 220, T.card, (*) => this.SetKey(Keys.Copilot))
        }
        ...
        UI.Link(g, (this.advanced ? "Masquer" : "Afficher") " les réglages avancés", M + 2, yAdv, 260, T.bg
            , (*) => (this.advanced := !this.advanced, UI.Later(() => this.Rebuild())))
        if this.advanced {
            fields := [["Nom affiché", "name", "Ex. Claude"], ["Processus", "process", "Ex. claude.exe"], ["Titre contient", "title", "Facultatif"]]
            for i, f in fields {
        ...
        ; --- boutons ---
        UI.Button(g, "Enregistrer", W - M - 130, yBtn, 130, "primary", T.bg, (*) => this.Save())
        UI.Button(g, "Annuler", W - M - 130 - 8 - 110, yBtn, 110, "secondary", T.bg, (*) => this.Close())
        if this.orig
            UI.Button(g, this.confirmDel ? "Confirmer" : "Supprimer", M, yBtn, 124, this.confirmDel ? "danger" : "subtle-danger", T.bg, (*) => this.Delete(), Chr(0xE74D))
    ...
    static Browse() {
        ...
        path := FileSelect(3, , "Choisir le programme à ouvrir", "Programmes (*.exe)")
        if path = ""
            return
        SplitPath(path, &file)
        d := this.draft
    ...
    static OnCaptured(hk, reason) {
        ...
        if reason = "timeout"
            this.msg := "Aucune touche détectée. Clique sur « Changer » pour réessayer."
        else if hk != ""
            this.TrySetKey(hk)
        UI.Later(() => this.Rebuild())
    }
    ...
    static TrySetKey(hk) {
        if (problem := Keys.Problem(hk)) != ""
            ...
            this.msg := "Cette touche est déjà utilisée par « " other.name " »."
        else
    ...
    static Save() {
        ...
        if d.target = "" || d.process = ""
            this.msg := "Choisis d'abord une application."
        else if d.key = ""
            this.msg := "Choisis une touche."
